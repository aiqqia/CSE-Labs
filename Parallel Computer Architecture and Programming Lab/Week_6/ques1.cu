...
__global__ void multiplyMatrixKernel_2d(int* A, int* B, int* C, int width){
	int col = threadIdx.x;
	int row = threadIdx.y;
	int k = 0;
	for (int i = 0; i < width; i++)
...
void multiplyMatrix(int* h_A, int* h_B, int* h_C, int width)
{
	...
	dimBlock.y = width;
	dimBlock.z = 1;
	dimGrid.x = ceil(width/2);
	dimGrid.y = ceil(width/2);
	dimGrid.z = 1;
	multiplyMatrixKernel_2d<<<dimGrid, dimBlock>>>(d_A, d_B, d_C, width);
	cudaMemcpy(h_C, d_C, size, cudaMemcpyDeviceToHost);
